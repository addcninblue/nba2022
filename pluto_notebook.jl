...
# ╔═╡ 4ba3f73a-ed2b-11ec-1356-696a8f8136b3
begin
	using Turing
	using Plots
end

# ╔═╡ ccc0a7ba-7e06-438b-a96e-ea75c146ae7d
begin
size = :small
if size == :small
	posterior = read("player_analysis_new.jls", Chains)
	players = [
		"K. Thompson"
		"J. Poole"
		"A. Wiggins"
...
elseif size == :mid
	posterior = read("player_analysis_mid.jls", Chains)
	players = [
		"S. Curry"
		"J. Poole"
		"M. Smart"
	...
	posterior = read("player_analysis_big.jls", Chains)
	players = [
		"A. Wiggins"
		"S. Curry"
		"K. Thompson"
...
# ╔═╡ 43713def-974d-4c32-bb04-20d8f60a9b8c
begin
...
# ╔═╡ 9e20a1ba-b5f3-462a-a8d9-c4773a7d2236
histogram(post_home, legend=false, title="Posterior distribution of home parameter")

# ╔═╡ 4d8b2e66-1296-4386-8019-cc8b4b86762a
begin
	att1 = 1
	histogram(post_att[att1], legend=false, title="Posterior distribution of $(players[att1])'s attack power")
end

# ╔═╡ 874bf980-1892-4e7b-a49f-c652d4f704f2
begin
	att2 = 17
	histogram(post_att[att2], legend=false, title="Posterior distribution of $(players[att2])'s attack power")
end

# ╔═╡ a7a2807f-6a0a-444a-ac1e-acece2cfd06b
begin
	def1 = 8
	histogram(post_def[def1], legend=false, title="Posterior distribution of $(players[def1])'s defense power")
end

# ╔═╡ e4651e13-a3e6-4266-a2f0-19b5220376be
begin
	def2 = 17
	histogram(post_def[def2], legend=false, title="Posterior distribution of $(players[def2])'s defense power")
end

# ╔═╡ b30cad1d-d27c-4add-bf22-83336fec1bda
begin
	def3 = 16
	histogram(post_def[def3], legend=false, title="Posterior distribution of $(players[def3])'s defense power")
end

# ╔═╡ a48aa43b-e3b9-475d-b4e7-6a21c1eeb356
mean.(post_def)

# ╔═╡ 060d4e4d-7f8e-498a-bf81-229325edb247
begin
	mean_post_att = mean.(post_att)
	std_post_att = std.(post_att)
	sorted_att = sortperm(mean_post_att)
	scatter(1:length(mean_post_att), mean_post_att[sorted_att], legend=false,  yerror=std_post_att[sorted_att])
	annotate!([(x, y+0.15, text(player, 4, :center, :black)) for (x, y, player) in zip(1:length(mean_post_att), mean_post_att[sorted_att], players[sorted_att])])
end

# ╔═╡ 3de21cc3-5d4b-4042-80dc-44a36cf946b8
begin
	gsw = ["S. Curry", "D. Green", "K. Looney", "K. Thompson", "A. Wiggins"]
	bos = ["J. Brown", "A. Horford", "M. Smart", "J. Tatum", "R. Williams"]
	gsw_starting_indices = findfirst.(isequal.(gsw), (players,))
	bos_starting_indices = findfirst.(isequal.(bos), (players,))

	(exp(sum(mean_post_att[gsw_starting_indices])),
	exp(sum(mean_post_att[bos_starting_indices])))
end
